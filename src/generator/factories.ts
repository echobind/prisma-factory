...
function addFactoryFunction(sourceFile: SourceFile, model: DMMF.Model) {
  ...
  newFunction.insertParameters(0, [
    { name: 'requiredAttrs', type: `Partial<${model.name}>`}
  ]);

  newFunction.setBodyText(`return createFactory<Prisma.${model.name}CreateInput, ${model.name}>('${model.name}', requiredAttrs); `)
